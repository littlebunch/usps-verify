...
use crate::schema::{derivations, food_groups, foods, manufacturers, nutrients, nutrient_data};
use crate::Get;
...
impl Nutdata_csv {
    fn create_nutdata(&self, fid:i32,conn:&MysqlConnection) -> Result<Nutrientdata, Box<dyn Error>> {
        //use crate::schema::foods::dsl::*;
...
pub fn process_nutdata(path: String, conn: &MysqlConnection) -> usize  {
    ...
    for n in ndcsv.records {
        ...
        nds.push(nd);
        if nds.len() % 2000 == 0 {
            count += insert_into(nutrient_data).values(&nds).execute(conn).unwrap();
            nds.clear();
        }
    }
    count += insert_into(nutrient_data).values(&nds).execute(conn).unwrap();
